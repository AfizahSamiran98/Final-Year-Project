...
$sql ="SELECT * FROM lawatan LEFT JOIN aktiviti
    ...
    if($result = mysqli_query($con,$sql)){
        if(mysqli_num_rows($result)>0){
            ...
            <tr>
			<td>ID Lawatan*</td>
			<td>:  <input name="id_lwtn" type="text" class="input-field" value="<?php echo htmlentities($row['id_lwtn']);?>" ></td>
			</tr>
			<tr>
			<td>Nama Pemohon</td>
			...
			<td>Status Lawatan</td>
                <td>  <div class="select-wrap one-third">
                : <select name="status_lwtn" id="status_lwtn" class="input-field" >
                   <?php
                if($result = mysqli_query($con,$sql));
             while($row=mysqli_fetch_array($result)):;?>
                  <option value="<?php echo htmlentities($row['status_lwtn']);?>"><?php echo $row ['status_lwtn'];?></option><?php endwhile;?>
			...
			<td>Staf Bertugas</td>
			<td><div class="select-wrap one-third">
                : <select name="emel_staf" class="input-field">
                <?php
                if($result = mysqli_query($con,$sql));
             while($row=mysqli_fetch_array($result)):;?>
                 <option value="<?php echo htmlentities($row['emel_staf']);?>"> <?php echo $row['nama_staf'];?></option><?php endwhile;?>
                <?php 
                $select= "select * FROM staf WHERE pengguna = 'staf' ORDER BY id_staf ASC";
                $selected = mysqli_query($con,$select);
                while($row = mysqli_fetch_array($selected)):;?>
                <option value="<?php echo $row['emel_staf'];?>">
                <?php echo $row['nama_staf'];?></option>
                <?php endwhile;?>
                </select>
                </div>
            </tr>
